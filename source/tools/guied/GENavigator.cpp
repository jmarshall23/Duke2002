...
LRESULT CALLBACK rvGENavigator::WndProc ( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	switch ( msg )
	{
		...
		case WM_CREATE:
		{
			...
			SetWindowLong ( hWnd, GWL_USERDATA, (intptr_t)nav );
			...
			SetWindowLong ( nav->mTree, GWL_USERDATA, (intptr_t)nav );
			SetWindowLong ( nav->mTree, GWL_WNDPROC, (intptr_t)ListWndProc );
