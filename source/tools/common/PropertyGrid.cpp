...
bool rvPropertyGrid::Create ( HWND parent, int id, int style )
{
	...
	SetWindowLong ( mWindow, GWL_USERDATA, (intptr_t)this );
	SetWindowLong ( mWindow, GWL_WNDPROC, (intptr_t)WndProc );
...
void rvPropertyGrid::FinishEdit ( void )
{
	...
	if ( !mEditLabel && item->mValue.Cmp ( value ) )
	{
		...
		if ( !SendMessage ( GetParent ( mWindow ), WM_NOTIFY, 0, (intptr_t)&nmpg ) )
		{
			mState = STATE_EDIT;
			SetFocus ( mEdit );
			return;
...
int rvPropertyGrid::AddItem ( const char* name, const char* value, EItemType type )
{
	...
	return SendMessage ( mWindow, LB_INSERTSTRING, insert, (intptr_t)item );
}
...
void rvPropertyGrid::RemoveAllItems ( void )
{
	...
	if ( mStyle & PGS_ALLOWINSERT )
	{
		...
		item->mName = "";
		item->mValue = "";
		SendMessage ( mWindow, LB_ADDSTRING, 0, (intptr_t)item );
	}
}
