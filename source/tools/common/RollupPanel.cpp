...
int rvRollupPanel::InsertItem ( const char* caption, HWND dialog, bool autoDestroy, int index )
{
	...
	SetWindowLong ( dialog, GWL_USERDATA,	(intptr_t)item );

	// Attach item to button through user data
	SetWindowLong ( button, GWL_USERDATA,	(intptr_t)item );
	SetWindowLong ( button, GWL_ID,			index );

	// Subclass dialog
	SetWindowLong ( dialog, DWL_DLGPROC, (intptr_t)DialogProc );

	// SubClass button
	SetWindowLong ( button, GWL_WNDPROC, (intptr_t)ButtonProc );
...
LRESULT CALLBACK rvRollupPanel::WindowProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch ( uMsg )
	{
		case WM_CREATE:
		{
			...
			SetWindowLong ( hWnd, GWL_USERDATA, (intptr_t)panel );
			break;
		}
